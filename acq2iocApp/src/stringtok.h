...
/** @file stringtok.h defines string Tokenizer */
...
template <typename Container> class Tokenizer{
/** tokenizes input string taking quotes into account. */
	...
	bool isws (char c)
	{
		if (c == quote){
			...
			return (strchr(ws,c) != NULL);
		}
	}
public:
	Tokenizer(string const &s, 
		  char const * const _ws = " \t\n",
		  char const _quote = '\'') : 
		in_quotes(0), ws(_ws), quote(_quote)
	{
		...
		while (i < S) {
			...
			while ((i < S) && (isws(s[i])))  ++i;
			if (i == S)  return;  // nothing left but WS

			// find end of word
			string::size_type  j = i+1;
			while ((j < S) && (!isws(s[j])))  ++j;
...
template <int I>
class StringBuffer {
/** closure class stores strings. */
	int maxlen;
	char lbuf[I];
	char* buf;
public:
	StringBuffer(int _maxlen){
		if (_maxlen < I){
			buf = lbuf;
			maxlen = I;
		}else{
			buf = new char[maxlen = _maxlen];
		}
	}
	~StringBuffer() {
		if (maxlen > I){
